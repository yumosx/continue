...
describe("Model Persistence User Flow", () => {
  ...
  test("should persist model choice exactly as user experiences it", async () => {
    ...
    const state = await modelService.initialize(mockAssistant, undefined);
    ...
    // persistModelName persists via GlobalContext
    ...
    const newState = await newModelService.initialize(mockAssistant, undefined);
